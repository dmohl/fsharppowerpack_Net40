...
set CHMOD=%CYGWIN%\chmod.exe
set PKZIP=%TOOLS%\win86\pkzip25.exe
if "%MSIDUMP%" == "" (
    set MSIDUMP=%TOOLS%\win86\msidump\x86\msidump.exe
)
set tree=FSharpPowerPack-1.9.9.9
...
for /F %%I in ('type zip.args') do %CHMOD% a=r %%I
del  FSharpPowerPack.zip
%PKZIP% -add -dir=current FSharpPowerPack.zip @zip.args
